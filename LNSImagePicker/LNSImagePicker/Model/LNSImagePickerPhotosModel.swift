//
//  LNSImagePickerPhotosModel.swift
//  LNSImagePicker
//
...
@available(iOS 8.0, *)
public class LNSImagePickerPhotosModel : LNSImagePickerModel {
    
    public var phasset:PHAsset!
    init(mediaType: LNSImagePickerMediaType,phasset:PHAsset) {
...
@available(iOS 8.0, *)
class LNSImagePickerPhotosAlbumModel:LNSImagePickerAlbumModel {
    ...
    override func getAlbumImage(size:CGSize) -> UIImage? {
        if let asset = self.result.object(at: 0) as? PHAsset {
            let model = LNSImagePickerPhotosModel(mediaType: .Photo, phasset: asset)
        ...
        DispatchQueue.global(qos: .default).async {
            self.result.enumerateObjects({ (asset, index, isStop) -> Void in
                if let phasset = asset as? PHAsset {
                    let mediaType:LNSImagePickerMediaType = phasset.mediaType == .image ? .Photo : .Video
                    let model = LNSImagePickerPhotosModel(mediaType: mediaType, phasset: phasset)
                    models.append(model)
                }
            })
