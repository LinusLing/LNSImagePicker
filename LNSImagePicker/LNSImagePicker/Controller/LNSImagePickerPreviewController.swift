...
//  
//
//  Created by linus on 5/11/19.
//  Copyright © 2019 linus. All rights reserved.
//
import UIKit
import AVFoundation

class LNSImagePickerPreviewController:UIViewController,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource {
    
    weak var delegate:LNSImagePickerDataSourceDelegate!
    var dataSource:[LNSImagePickerModel]!
    ...
    class var instance:LNSImagePickerPreviewController {
        get {
            let storyboard = UIStoryboard(name: "LNSImagePicker", bundle: Bundle.getResourcesBundle())
            let vc = storyboard.instantiateViewController(withIdentifier: "LNSImagePickerPreviewController") as! LNSImagePickerPreviewController
            return vc
        }
    }
    ...
    override func willRotate(to toInterfaceOrientation: UIInterfaceOrientation, duration: TimeInterval) {
        if UIApplication.shared.statusBarOrientation.isPortrait != toInterfaceOrientation.isPortrait {
            ...
            self.collectionView.prevItemSize = (self.collectionView.collectionViewLayout as! LNSImagePickerPreviewFlowLayout).itemSize
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        self.perform(#selector(LNSImagePickerPreviewController.didEndDecelerating), with: nil, afterDelay: 0)
    }
